    ...
    <div className="pt-4 border-t border-neutral-800">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex space-x-3">
          ...
          <button type="button" onClick={() => append({ description: "", hours: 0, cost: 0, amount: 0, isSection: false })} className="text-indigo-400 text-sm flex items-center hover:text-indigo-300"><Plus className="w-4 h-4 mr-1" /> Add Row</button>
        </div>
      </div>
      ...
      <div className="mt-6 flex justify-end">
        <div className="w-64 space-y-3">
          <div className="flex justify-between text-sm text-neutral-400">
            ...
            <span>{watchLineItems.reduce((sum: number, item: any) => sum + (item.isSection ? 0 : (item.hours || 0)), 0)}</span>
          </div>
