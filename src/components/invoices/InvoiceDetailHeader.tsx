...
export function InvoiceDetailHeader({ invoice, toggleStar, handleDelete, isDeleting, handleStatusChange }: InvoiceDetailHeaderProps) {
  ...
  const safeClientName = (invoice.client?.name || 'Invoice').replace(/[^a-zA-Z0-9]/g, '_');
  const defaultFileName = `${invoice.invoiceNumber || safeClientName}`;
  ...
  const handleShare = (option: string) => {
    ...
    else if (option === 'pdf') setIsDownloadOpen(true);
  };
        ...
        <div className="flex space-x-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48 bg-neutral-900 border-neutral-800">
              ...
              <DropdownMenuItem onClick={() => setIsDownloadOpen(true)} className="hover:bg-neutral-800 cursor-pointer text-sm"><Download className="w-4 h-4 mr-2" /> Download PDF</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
