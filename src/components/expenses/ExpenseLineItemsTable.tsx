...
export function ExpenseLineItemsTable({ subtotal, total, taxRate }: { subtotal: number, total: number, taxRate: number }) {
    ...
    <div className="pt-4 border-t border-neutral-800">
      ...
      <div className="mt-6 flex justify-end">
        <div className="w-64 space-y-3">
          <div className="flex justify-between font-medium text-sm"><span>Subtotal</span><span>${subtotal.toFixed(2)}</span></div>
          ...
          <div className="flex justify-between font-medium text-lg border-t border-neutral-800 pt-3"><span>Total</span><span>${total.toFixed(2)}</span></div>
        </div>
      </div>
    </div>
  );
