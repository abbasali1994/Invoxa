...
import { DownloadPdfModal } from "@/components/shared/DownloadPdfModal";

export function ExpenseDetailHeader({ expense, id, handleDelete }: { expense: any, id: string, handleDelete: () => void }) {
  const [isDownloadOpen, setIsDownloadOpen] = React.useState(false);

  const safeVendor = (expense.vendor || 'Expense').replace(/[^a-zA-Z0-9]/g, '_');
  const defaultFileName = `${expense.expenseNumber || safeVendor}`;
  ...
  const handleShare = (type: string) => {
    ...
    switch (type) {
      ...
      case 'pdf': setIsDownloadOpen(true); break;
    }
  };
  ...
  return (
    <>
        ...
        <div className="flex space-x-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48 bg-neutral-900 border-neutral-800 text-neutral-200">
              ...
              <DropdownMenuItem onClick={() => setIsDownloadOpen(true)} className="hover:bg-neutral-800 cursor-pointer"><Download className="w-4 h-4 mr-2" /> Download PDF</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>

          <button onClick={() => setIsDownloadOpen(true)} className="flex items-center px-4 py-2 bg-indigo-600 rounded-md text-sm font-medium hover:bg-indigo-700 transition-colors">
      ...
      <DownloadPdfModal
        ...
        downloadUrl={`/api/expenses/${id}/pdf`}
        title={`Download Expense (${expense.vendor})`}
      />
    </>
  );
}
